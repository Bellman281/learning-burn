...
const ST: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../../weights/small_cnn.safetensors"
);

// The same name mapping as Example 19.1.
fn store() -> SafetensorsStore {
    SafetensorsStore::from_file(ST)
        .with_key_remapping(r"^features\.0\.(.+)$", "conv1.$1")
        .with_key_remapping(r"^features\.3\.(.+)$", "conv2.$1")
        .with_key_remapping(r"^classifier\.(.+)$", "head.$1")
}

/// Returns (error without the adapter, head weight shape, predictions, max |Burn - PyTorch|).
fn build() -> (String, [usize; 2], Vec<i64>, f32) {
    let device = Default::default();

    // 1. Names mapped, but no adapter: safetensors is just named arrays, in PyTorch's layout.
    let mut model = SmallCnn::<NdArray>::new(&device);
    let error = match model.load_from(&mut store()) {
        Ok(_) => "loaded?!".to_string(),
        Err(e) => e.to_string(),
    };

    // 2. With the adapter that turns PyTorch's layout into Burn's.
    let mut model = SmallCnn::<NdArray>::new(&device);
    model
        .load_from(&mut store().with_from_adapter(PyTorchToBurnAdapter))
        .expect("load safetensors");

    let shape = model.head.weight.dims();
    ...
    (error, shape, predicted, max_diff(&values, &reference()))
}

fn main() {
    let (error, shape, predicted, diff) = build();
    println!("without the adapter: {error}");
    println!();
    println!("with PyTorchToBurnAdapter:");
    println!("  head.weight shape  = {shape:?}   (PyTorch stored [4, 8])");
...
mod tests {
    ...
    #[test]
    fn matches_pytorch() {
        let (_, shape, predicted, diff) = build();
        assert_eq!(shape, [8, 4]);
